/**
 * Full SDK Test Script (CommonJS)
 * Tests mock data, config validation, and error handling
 */
...
console.log('\n========================================');
console.log('  FLOWSTACK SDK FULL TEST SUITE');
console.log('========================================\n');
...
test('validateConfig fails without jwtSecret', () => {
  const result = validateConfig({
    passwordSecret: 'test-password',
  });
...
test('validateConfig fails without passwordSecret', () => {
  const result = validateConfig({
    jwtSecret: 'test-secret',
  });
  assert(!result.valid, 'Should be invalid');
...
test('validateConfig warns about missing tenantId', () => {
  const result = validateConfig({
    jwtSecret: 'test-secret',
    passwordSecret: 'test-password',
  });
...
test('validateConfig accepts mock mode', () => {
  const result = validateConfig({
    jwtSecret: 'test-secret',
    passwordSecret: 'test-password',
    mode: 'mock',
  });
  assert(result.valid, 'Should be valid with mock mode');
});

// ==================== ERROR HANDLING TESTS ====================
console.log('\n❌ Error Handling Tests\n');

test('FlowstackError can be created', () => {
  const error = new FlowstackError('TEST_ERROR', 'Test message', {
    userMessage: 'User-friendly message',
    recoveryAction: 'Try again',
  });
  assert(error.code === 'TEST_ERROR', 'Wrong code');
  assert(error.message === 'Test message', 'Wrong message');
  assert(error.userMessage === 'User-friendly message', 'Wrong userMessage');
  assert(error.recoveryAction === 'Try again', 'Wrong recoveryAction');
});

test('FlowstackError is instanceof Error', () => {
  const error = new FlowstackError('TEST', 'Test');
  assert(error instanceof Error, 'Should be instance of Error');
});

test('isFlowstackError identifies FlowstackError', () => {
  const error = new FlowstackError('TEST', 'Test');
  assert(isFlowstackError(error), 'Should identify FlowstackError');
  assert(!isFlowstackError(new Error('regular')), 'Should not identify regular Error');
  assert(!isFlowstackError(null), 'Should not identify null');
  assert(!isFlowstackError('string'), 'Should not identify string');
});

test('ErrorCodes has expected codes', () => {
  assert(ErrorCodes.NETWORK_ERROR, 'Missing NETWORK_ERROR');
  assert(ErrorCodes.AUTHENTICATION_FAILED, 'Missing AUTHENTICATION_FAILED');
  assert(ErrorCodes.WORKSPACE_NOT_FOUND, 'Missing WORKSPACE_NOT_FOUND');
  assert(ErrorCodes.RATE_LIMITED, 'Missing RATE_LIMITED');
  assert(ErrorCodes.CONFIG_INVALID, 'Missing CONFIG_INVALID');
});
...
test('UserActivityLog has expected activity types', () => {
  const types = mockUserActivity.map(a => a.activityType);
  assert(types.includes('login'), 'Missing login type');
});
